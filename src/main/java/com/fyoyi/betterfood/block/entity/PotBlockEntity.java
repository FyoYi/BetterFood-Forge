...
public class PotBlockEntity extends BlockEntity implements Container {
    ...
    private void applyHeat() {
        ...
        if (hasItems || this.hasOil) {
            ...
            if (this.level.getGameTime() % 2 == 0) {
                ...
                if (rng < 0.3f) {
                    this.level.playSound(null, this.worldPosition, SoundEvents.GENERIC_EXTINGUISH_FIRE,
                            SoundSource.BLOCKS, 0.065F + hasOilSoundAdd,
                            2.5F + (this.level.random.nextFloat() - 0.5F) * 0.5F);
                } else if (rng < 0.6f) {
                    this.level.playSound(null, this.worldPosition, SoundEvents.REDSTONE_TORCH_BURNOUT,
                            SoundSource.BLOCKS, 0.085F + hasOilSoundAdd,
                            2.0F + (this.level.random.nextFloat() - 0.5F) * 0.5F);
                } else if (rng < 0.9f) {
                    this.level.playSound(null, this.worldPosition, SoundEvents.CANDLE_EXTINGUISH, SoundSource.BLOCKS,
                            0.1F + hasOilSoundAdd, 2.0F);
                }
            }
            if (this.level.getGameTime() % 10 == 0 && this.level.random.nextFloat() < 0.1f && this.hasOil) {
                this.level.playSound(null, this.worldPosition, SoundEvents.LAVA_POP, SoundSource.BLOCKS, 0.5F,
        ...
        if (this.level.getGameTime() % 20 == 0 && this.level.random.nextFloat() < 0.7f) {
            this.level.playSound(null, this.worldPosition, SoundEvents.FIRE_AMBIENT, SoundSource.BLOCKS, 0.5F,
                    1.0F + this.level.random.nextFloat() * 0.4F);
        }
    }
    ...
    private void spawnSustainedOilEffects() {
        if (level instanceof ServerLevel serverLevel) {
            ...
            if (this.oilSizzleTimer % 3 == 0) {
                ...
                level.playSound(null, pos, SoundEvents.GENERIC_EXTINGUISH_FIRE, SoundSource.BLOCKS, volume,
                        2.6F + (level.random.nextFloat() - 0.5f) * 0.4f);
            }
        }
    }
            ...
            if (items.get(i).isEmpty()) {
                ...
                if (level != null && !level.isClientSide) {
                    if (isHeated(this.level, this.worldPosition)) {
                        level.playSound(null, this.worldPosition, SoundEvents.GENERIC_EXTINGUISH_FIRE,
    ...
    public ItemStack serveDish() {
        ...
        // 记录菜谱到玩家菜谱书
        if (!level.isClientSide && matchedRecipe != null) {
            recordRecipeToPlayers(result, matchedRecipe);
        }
     ...
     * 记录菜谱到附近的玩家
     */
    private void recordRecipeToPlayers(ItemStack result, CulinaryRecipe recipe) {
        if (!(level instanceof ServerLevel serverLevel))
            return;

        // 获取评分
        CompoundTag tag = result.getTag();
        float score = tag != null ? tag.getFloat("DishScore") : 0f;
        String dishName = result.getHoverName().getString();

        // 构建食材需求列表
        java.util.List<com.fyoyi.betterfood.recipe.RecipeBook.IngredientRequirement> ingredients = new java.util.ArrayList<>();
        for (CulinaryRecipe.Requirement req : recipe.getRequirements()) {
            float tolerance = req.tolerance;
            float minCook = req.idealCookedness - tolerance;
            float maxCook = req.idealCookedness + tolerance;
            ingredients.add(new com.fyoyi.betterfood.recipe.RecipeBook.IngredientRequirement(
                    req.classification, minCook, maxCook));
        }

        // 为附近的玩家发送事件
        for (Player player : serverLevel.getPlayers(p -> p.distanceToSqr(worldPosition.getCenter()) <= 64.0)) {
            com.fyoyi.betterfood.event.RecipeUnlockEvent event = new com.fyoyi.betterfood.event.RecipeUnlockEvent(
                    player,
                    com.fyoyi.betterfood.event.RecipeUnlockEvent.RecipeType.COOKING_PAN,
                    dishName,
                    score,
                    ingredients);
    ...
    private float calculateAverageFreshness() {
        ...
        for (ItemStack s : items) {
            if (s.isEmpty())
                continue;
            total += (FreshnessHelper.getFreshnessPercentage(this.level, s) * 100f >= 95f ? 100f
                    : (FreshnessHelper.getFreshnessPercentage(this.level, s) * 100f / 95f) * 95f);
            count++;
        }
    ...
    public void load(CompoundTag t) {
        ...
        if (t.contains("HasOil"))
            hasOil = t.getBoolean("HasOil"); // 读取油
    }
